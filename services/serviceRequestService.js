...
const MAX_LIMIT = 200;
const GENERATED_DOCUMENT_FIELD_KEY = "generated_document";
...
function publicUrlForS3Key(key) {
  ...
  const cdnDomain = process.env.PUBLIC_CDN_DOMAIN || null;
...
function htmlToPlainText(html) {
  return decodeHtmlEntities(html)
    ...
    .replace(/<\s*li[^>]*>/gi, "• ")
    .replace(/<\/\s*li\s*>/gi, "\n")
    .replace(/<\/\s*(div|section|article|tr)\s*>/gi, "\n")
    .replace(/<[^>]+>/g, "")
    .replace(/\n{3,}/g, "\n\n")
    .replace(/[ \t]{2,}/g, " ")
    .trim();
...
function answerDisplayValue(answer) {
  ...
  if (answer.value_json !== null && answer.value_json !== undefined) {
    return typeof answer.value_json === "object"
      ? JSON.stringify(answer.value_json)
      : String(answer.value_json);
  }
  return "";
}

function buildTemplateContext(service, requestDetails, payload) {
  const context = {
    service_title: service.title,
    variant_title: requestDetails.variant?.title || "",
    profile_title: normalizeString(payload?.profile_title),
  ...
  for (const answer of requestDetails.answers || []) {
    context[answer.field_key] = answerDisplayValue(answer);
  }
...
function createPdfBuffer({ title, profileTitle, renderedHtml }) {
  return new Promise((resolve, reject) => {
    ...
    doc.font("Helvetica-Bold").fontSize(18).fillColor("#202026").text(title, {
      align: "center",
    });
    if (profileTitle) {
      doc.moveDown(0.35);
      doc.font("Helvetica").fontSize(9).fillColor("#66666F").text(`Profile: ${profileTitle}`, {
        align: "center",
      });
    }
    doc.moveDown(1.5);
    doc.font("Helvetica").fontSize(11).fillColor("#202026").text(htmlToPlainText(renderedHtml), {
      align: "left",
      lineGap: 4,
    });
    doc.moveDown(2);
    doc.fontSize(8).fillColor("#8A8A92").text("Generated by Kaptaan", {
      align: "center",
    });
...
async function uploadGeneratedDocumentPdf(buffer, filename) {
  const region = process.env.AWS_DEFAULT_REGION;
  const bucket = process.env.S3_BUCKET;

  if (!region || !bucket) {
    throw createValidationError("Document storage is not configured");
  }

  const s3 = new S3Client({ region });
  const key = `mobile/documents/${folderByDate()}${crypto.randomBytes(8).toString("hex")}.pdf`;
  ...
  return {
    key,
    public_url: publicUrlForS3Key(key),
  };
}
...
async function generateFreeDocument(userId, payload) {
  const serviceId = normalizePositiveInteger(payload?.service_id, "service_id");
  const service = await getServiceById(serviceId);
  if (!service || service.status !== "published" || service.service_type !== "documents") {
    throw createValidationError("The selected document is not available");
  }

  const variantId = normalizePositiveInteger(payload?.service_variant_id, "service_variant_id");
  const variant = (service.variants || []).find((item) => item.id === variantId && item.is_active);
  if (!variant || Number(variant.price_paise || 0) !== 0) {
    throw createValidationError("Only free document variants can be generated");
  }

  if (!normalizeString(service.document_template_html)) {
  ...
  const checkout = await createServiceCheckout(userId, {
  ...
  const context = buildTemplateContext(service, requestDetails, payload);
  const renderedHtml = renderTemplateHtml(service.document_template_html, context);
  const filename = `${sanitizeFilenamePart(service.title)}-${requestDetails.id}.pdf`;
  const pdfBuffer = await createPdfBuffer({
    title: service.title,
    profileTitle: context.profile_title,
    renderedHtml,
  });
  const upload = await uploadGeneratedDocumentPdf(pdfBuffer, filename);
  ...
  await pool.query(
    `INSERT INTO service_request_files (
     ...
     VALUES ($1, $2, $3, $4, 'application/pdf', 999)`,
    [requestDetails.id, GENERATED_DOCUMENT_FIELD_KEY, upload.public_url, filename],
  );

  await pool.query(
    `UPDATE services
     SET document_download_count = document_download_count + 1,
         updated_at = CURRENT_TIMESTAMP
     WHERE id = $1`,
    [serviceId],
  );
