...
docker exec -i postgres-db psql -U mridul -d kaptaanAPI < db/schema/session_pause_columns.sql
...
docker exec -i postgres-db psql -U mridul -d kaptaanAPI < db/schema/tasks.sql
